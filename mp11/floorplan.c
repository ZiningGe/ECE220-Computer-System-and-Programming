 ...
 * Simon Ge
 *
 * This MP completes several fundamental functions for optimizing a circuit.
 * There are three main stages of functions
 * is_leaf_node, is_internal_node, is_subtree check theh status of a node to assign specific tasks
 * rotate, recut, swap_module, swap_topology are functions change the design of the circuit to achieve its most optimized floorplan
 * postfix_traversal, get_total_resources, inti_slicing_tree are corresponding to the overall structure of the data
 *
 *
...
int is_leaf_node(node_t* ptr) {
    return (ptr->left == NULL && ptr->right == NULL); //check if the left and right children are both NULL
}
...
int is_internal_node(node_t* ptr) {
    ...
    return !(ptr->left == NULL && ptr->right == NULL); //check if the left and right children are neither NULL
}
...
int is_in_subtree(node_t* a, node_t* b) {
    ...
    if (a == b || a->left == b || a->right == b){
        return 1; // check whether b is in the subtree of a
    }
    else if (a->left == NULL && a->right == NULL){
        return 0; // check if the program reaches the end
    }

    return is_in_subtree(a->left, b) || is_in_subtree(a->left, b); // recursive step
}
...
void rotate(node_t* ptr) {
    if (!is_leaf_node(ptr))
        return; // return if the node is an internal node
    int temp = ptr->module->h;
    ptr->module->h = ptr->module->w;
    ptr->module->w = temp; // swap height and width
    return;
}
...
void recut(node_t* ptr) {
    ...
    // TODO:
    if (ptr->cutline == V){
        ptr->cutline = H; // swap vertical to horizontal
    }
    else{
        ptr->cutline = V; // swap horizontal to vertical
    }
...
void swap_topology(node_t* a, node_t* b) {
  ...
  assert(a->parent != NULL && b->parent != NULL);
    ...
    node_t* ap_ptr = a->parent;
    node_t* bp_ptr = b->parent;
    
    if (ap_ptr->left == a && bp_ptr->left == b) { // a and b are both left children
        node_t* temp = ap_ptr->left;
        ap_ptr->left = bp_ptr->left;
        bp_ptr->left = temp; // swap (case 1)
    }
    else if (ap_ptr->left == a && bp_ptr->right == b) { // a is the left child and b is the right child
        node_t* temp = ap_ptr->left;
        ap_ptr->left = bp_ptr->right;
        bp_ptr->right = temp; // swap (case 2)
    }
    else if (ap_ptr->right == a && bp_ptr->right == b) { // a and b are both right children
        node_t* temp = ap_ptr->right;
        ap_ptr->right = bp_ptr->right;
        bp_ptr->right = temp; // swap (case 3)
    }
    else { // (ap_ptr->right == a && bp_ptr->left == b) // a is the right child and b is the left child
        node_t* temp = ap_ptr->right;
        ap_ptr->right = bp_ptr->left;
        bp_ptr->left = temp; // swap (case 4)
    }
    
    node_t* temp = a->parent;
    a->parent = b->parent;
    b->parent = temp; // swap parents
}
...
void postfix_traversal(node_t* ptr, int* nth, expression_unit_t* expression) {
    ...
    postfix_traversal(ptr->left, nth, expression); // traversal left first
    postfix_traversal(ptr->right, nth, expression); // traversal righr
    
    expression[*nth].module = ptr->module; //store  module to expression
    expression[*nth].cutline = ptr->cutline; //store  cutline type to expression
    
//    if (ptr->module == NULL){
//        printf("V");
//    }
//    else{
//        printf("%d", ptr->module->resource);
//    }
//
    *nth = *nth + 1; // increment index
}
...
int get_total_resource(node_t* ptr)
{
    // TODO:
    if (ptr->module != NULL){
        return ptr->module->resource; // base case, addd resources
    }
    return get_total_resource(ptr->right) + get_total_resource(ptr->left); //recursive step
}
...
node_t* init_slicing_tree(node_t* par, int n) {
    ...
    // Base case
    if (n == num_modules-2){
        node_t * node_left_ptr = (node_t *)malloc(sizeof(node_t));
        node_t * node_right_ptr = (node_t *)malloc(sizeof(node_t));
        // Initialize the right leaf node
        node_right_ptr->module = &modules[n];
        node_right_ptr->cutline = UNDEFINED_CUTLINE;
        node_right_ptr->parent = par;
        node_right_ptr->left = NULL;
        node_right_ptr->right = NULL;
        // Initialize the left leaf node
        node_left_ptr->module = &modules[n+1];
        node_left_ptr->cutline = UNDEFINED_CUTLINE;
        node_left_ptr->parent = par;
        node_left_ptr->left = NULL;
        node_left_ptr->right = NULL;
        // Update the parent's left and right children
        par->right = node_right_ptr;
        par->left = node_left_ptr;
        return NULL;
    }
    ...
    if (par == NULL && n == 0){
        ...
        node_ptr->module = NULL;
        node_ptr->cutline = V;
        node_ptr->parent = NULL;
        node_ptr->left = NULL;
        node_ptr->right = NULL;
        // Recursively call function init_slicing_tree()
        init_slicing_tree(node_ptr,n);
        return node_ptr;
    }
    else{
        node_t * node_left_ptr = (node_t *)malloc(sizeof(node_t));
        ...
        node_right_ptr->right = NULL;
        // Initialize the left leaf node
        node_left_ptr->module = NULL;
        node_left_ptr->cutline = V;
        node_left_ptr->parent = par;
        node_left_ptr->left = NULL;
        node_left_ptr->right = NULL;
        // Update the parent's left and right children
        ...
        par->left = node_left_ptr;
        // Recursively call function init_slicing_tree()
        init_slicing_tree(node_left_ptr,n+1);
        return NULL;
    }
