...
int main(){
	...
	cell temparray[12] = {4, 2, 8, -1, 2, -1, -1, -1, 16, -1, -1, -1};
	for( i =0; i<12;i++){
	...
	if( move_s(game_cpy1) == _move_s(game_cpy2))
		printf("test 15: pass - move_s returns the correct value\n");
	else
		printf("test 15: fail - move_s returns the incorrect value\n");
	...
	for( i = 0; i < 12; i ++){
		if (*ptr1 != *ptr2){
			printf("test 15: fail - move_s does not move tiles correctly\n");
			//exit(0);
			printf("Your output:");
			print_game(game_cpy1);
			printf("Correct output:");
	...
	printf("test 15: pass - move_s moves the tiles around correctly\n");
	print_game(game_cpy1);
	
	if (legal_move_check(game_cpy1) == _legal_move_check(game_cpy2)){
		printf("test 16: pass - legal_move_check functions correctly\n");
		printf("%d\n", legal_move_check(game_cpy1));
		print_game(game_cpy1);
	}
